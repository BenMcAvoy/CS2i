...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImVec2 pos = window->DC.CursorPos;
    const ImRect total_bb(pos, pos + ImVec2(square_sz + (label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f), label_size.y + style.FramePadding.y * 2.f));
    ItemSize(total_bb, style.FramePadding.y);
    ...
    auto it_anim = anim_states.find(id);
    if (it_anim == anim_states.end())
    {
        anim_states.insert({ id, 0.0f });
        it_anim = anim_states.find(id);
    }

    // Update the animation state using linear interpolation
    float target_value = *v ? 1.0f : 0.0f;
    it_anim->second = ImLerp(it_anim->second, target_value, 0.08f);

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));
    if (IsItemVisible())
    {
        window->DrawList->AddRectFilled(check_bb.Min, check_bb.Max, GetColorU32(ImGuiCol_FrameBg), style.FrameRounding);

        const float half_fill_sz = (it_anim->second * square_sz) / 2.0f;
        const ImVec2 center = ImVec2((check_bb.Min.x + check_bb.Max.x) / 2.0f, (check_bb.Min.y + check_bb.Max.y) / 2.0f);
        ImVec2 fill_bb_min = ImVec2(center.x - half_fill_sz, center.y - half_fill_sz);
        ImVec2 fill_bb_max = ImVec2(center.x + half_fill_sz, center.y + half_fill_sz);
        window->DrawList->AddRectFilled(fill_bb_min, fill_bb_max, GetColorU32(ImGuiCol_CheckMark), style.FrameRounding);
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImRect frame_bb(total_bb.Min + ImVec2(0, label_size.y + style.FramePadding.y), total_bb.Max);*/

    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect total_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + 10));
    const ImRect frame_bb(total_bb.Min + ImVec2(0, label_size.y), total_bb.Max);
    ...
    if (!temp_input_is_active)
    {
        ...
        // Store initial value (not used by main lib but available as a convenience)
        if (make_active)
            memcpy(&g.ActiveIdValueOnActivation, p_data, DataTypeGetInfo(data_type)->Size);
    ...
    float fill_fraction = (*reinterpret_cast<const float*>(p_data) - *reinterpret_cast<const float*>(p_min)) /
        (*reinterpret_cast<const float*>(p_max) - *reinterpret_cast<const float*>(p_min));

    float fill_width = frame_bb.Min.x + fill_fraction * (frame_bb.Max.x - frame_bb.Min.x);
    ...
    it_anim->second.opacity = ImLerp(it_anim->second.opacity, IsItemActive() ? 1.0f : 0.4f, 0.08f * (1.0f - g.IO.DeltaTime));


    ImVec4 fillColor = ImGui::GetStyle().Colors[ImGuiCol_SliderGrab];
    ...
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(fill_width, frame_bb.Max.y), ImColor(fillColor), 0.5f); 
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 && (flags & ImGuiColorEditFlags_NoInputs) == 0)
    {
        ...
        for (int n = 0; n < components; n++)
        {
            ...
            if (!(flags & ImGuiColorEditFlags_NoOptions))
                OpenPopupOnItemClick("pContext", ImGuiPopupFlags_MouseButtonRight);
        }
    }
...
void ImGui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags)
{
    ...
    if ((!allow_opt_inputs && !allow_opt_datatype) || !BeginPopup("pContext"))
        return;
...
void ImGui::ColorPickerOptionsPopup(const float* ref_col, ImGuiColorEditFlags flags)
{
    ...
    if ((!allow_opt_picker && !allow_opt_alpha_bar) || !BeginPopup("pContext"))
        return;
